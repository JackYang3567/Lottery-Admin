...
function moneyAction($config = []){
  ...
  type 0:下注 1;提现 2:线下充值 3:中奖 4:扣款 5:赠送 6:退款 7:在线充值 8:返佣 9:签到
      ...
      18:日负反水,19:游戏上分,20:游戏下分,21:api游戏押注返还
  */
  $return_data = [
    'code' => 0,
    'msg' => ''
  ...
  foreach ($config as $value) {
    try{
      $value += [
        ...
        // 操作金额
        'money' => 0,
        // 0:下注 1;提现 2:线下充值 3:中奖 4:扣款 5:赠送 6:退款 7:在线充值 8:返佣 9:签到 10:抽奖 11:返水 12:冻结 13:冻结返还 14:保底投注 15:系统充值 16:聊天室红包 17:个人充值红包
        'type' => 'no',
        // 说明(写入资金明细表的说明)
        'explain' => null
      ];
...
//            Db::execute('update user set money=money -'.$value['money'].' where money -'.$value['money'].' > 0 and id='.$value['uid'])
//            $model->setDec($value1[0], $value['money']);
            //echo 'update user set '.$value1[0].'='.$value1[0].' -'.$value['money'].' where '.$value1[0].' -'.$value['money'].' > 0 and id='.$value['uid'];
  ...
  foreach ($data as &$value) {
    ...
    if(isset($data_chat['type_'.$value['type']])){
      ...
      if($value['type'] == 35){
        ...
        // print_r($value['type']);
        $lottery_config = Db::table('lottery_config')->field('basic_config,official,name')->where(['type'=>$value['type']])->find();
        $lottery_config['basic_config'] = $data[0]['category'] == 0 ? json_decode($lottery_config['basic_config'],true):	json_decode($lottery_config['official'],true);
...
function gradeJudgement($id){
    ...
    $ss =  DB::table('accumulation')->where('user_id',$id)->field('(in_money+online_money) as chongzhi,use_money')->order('create_time desc')->find();
    if (empty($ss)){
        return 1;
    }
    $dengji = Db::table('user_rank')->order('rank desc')->select();
    $user_grade = Db::table('user')->where('id',$id)->find(); //用户设置的等级
    foreach ($dengji as $k =>$v ){
        $sj = json_decode($v['condition'],true);
        if ($ss['chongzhi'] >= $sj['recharge'] ){
            if ( $ss['use_money'] >= $sj['account']){
                $dengji_x =  $v['rank']; //计算后的等级
            }
        }
    }
    if ($dengji_x > $user_grade['grade'] ){
        Db::table('user')->where('id',$id)->update(['grade' =>$dengji_x ]);
        return $dengji_x;
    }else{
