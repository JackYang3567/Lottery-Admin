...
class Betting extends Model
{
   ...
   * 获取动态内容
   */
  public function getChgconAttr($value,$data){
    $bit = User::get($data['user_id']);
    $strl = mb_strlen($bit->username);
    $lname = LotteryConfig::get($data['type'])['name'];
    $rs = [
      'username' => $bit->type == 1 ? '试玩用户' :(mb_substr($bit->username,0,floor($strl/2)).'**'),
      'lottery' => $lname
    ];
    return $rs;
  }

  static public function uAction(){
    $rs = self::field('user_id,type,win,create_time')->where('state', ['=', 1], ['=', 2], 'or')->where('win','>',0)->order('id','DESC')->limit(10)->select();
    return $rs->append(['chgcon'])->hidden(['user_id','type']);
  }
  /**
   * 获取器28系列,投注数据修改
   * @return array
