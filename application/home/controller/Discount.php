...
class Discount extends Common
{
  public function getInfo(){
    $data = input('post.');
    $model = Db::table('article')->field('id,title,cat_id')->order('id DESC');
    if(!empty($data) && isset($data['id'])){
      $data = $model->where('id','>',$data['id'])->select();
    }else{
  ...
  public function getChatRoom(){
    ...
    $id = input('post.id');
    $data = Db::table('chat_room')->where([ 'id'=>['id','gt',$id] ])->limit(20)->order('id DESC')->select();
    if($data){
      foreach ($data as &$value) {
        ...
        if($value['type'] == 1){
          ...
          $value['photo'] = User::get($value['user_id'])->photo;
        }elseif($value['type'] == 2){
          if($value['user_id'] == 0){
            $value['user_name'] = '系统';
          }else{
