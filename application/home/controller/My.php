    ...
    if(!empty($user_password['draw_password']) ? md5($get_data['password']) == $user_password['draw_password'] : md5($get_data['password'])==$this->user['password']){
      ...
      if(isset($get_data['type']) && $get_data['type'] == 1){
        if((new UserInfo)->save(['user_id'=>$this->user['id'],'phone_number'=>$get_data['phone']],(UserInfo::field('user_id')->where(['user_id'=>$this->user['id']])->find()?true:false) )){
          return ['code'=>1,'msg'=>'手机号码更新成功'];
        }else{
          return ['code'=>-2,'msg'=>'手机号码更新失败'];
        }
      }
      ...
      if(UserBank::where('user_id','=',$this->user['id'])->find()){
        return ['code'=>-2,'msg'=>'已绑定无法,再次绑定'];
      };
...
// print_r($where);die;
  ...
  public function sign(){
    ...
    if($user['type'] == 1){
      ...
      if($user['group'] == 1){
        $return_data['code'] = -3;
        $return_data['msg'] = '您的签到积分功能已被取消,如有问题请联系客服';
 ...
 */
  ...
  public function Verification_uname(){
    $data = input('post.');
    $uname = $data['uname'];
    $res = Db::table('user_bank')->where('username',$uname)->find();
    return $res;
  }
