...
//			dump($lottery_trade);
		...
		$sid = [4,7,12,13,16,18,19,20,22,23,26,27,28,30,33,34,32,43,48,50,54,55,0,1,29,35,53,56];
		$list = DB::table('lottery_config')->where('type','not in',$sid)->where($map)->field('name,type,switch,time_config,note')->order('type ASC')->paginate($paginate, false, $pageParam)->each(function ($item, $key) {
...
//            dump(Request::param());
			...
			}elseif (Request::param('type_id') == 3 ||Request::param('type_id') == 5){//判断是 pk10
	...
	public function showSetting() {
		if (Request::method() == 'GET') {
			$sid = [4,7,12,13,16,18,19,20,22,23,26,27,28,30,33,34,32,43,48,50,54,55,29,35,53,56];
			 ...
			 $loc = DB::table('lottery_config')->where('type','not in',$sid)->order('type ASC')->column('type,name');
			...
			$rs = Db::table('lottery_config')->where('type','not in',$sid)->column('type');
...
//                dump($return_config['return']);
	...
	public function gcSetting() {
		if (Request::method() == 'GET') {
			$sid = [4,7,12,13,16,18,19,20,22,23,26,27,28,30,33,34,32,43,48,50,54,55,0,1,29,35,53,56];
			$list = DB::table('lottery_config')->where('type','not in',$sid)->field('type,name')->order('type ASC')->select();
