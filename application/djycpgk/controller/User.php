...
//        dump(Request::param('id'));
    ...
    public function index() {
        ...
        if (isset($data['agent']) && $data['agent']!=''){ //代理名称
            $agent = $data['agent'];
            $proxy_id = Proxy::where('username',$data['agent'])->field('uid')->find()['uid'];
            $where[] = ['proxy_id', '=',$proxy_id?$proxy_id:'' ];
        }
        if (isset($data['start_time']) && $data['start_time']!='') { //开始时间
            $start_time = $data['start_time'];
            $where[] = ['create_time','>',strtotime($data['start_time'])];
        }
        if (isset($data['end_time']) && $data['end_time']!='') { //结束时间
            $end_time = $data['end_time'];
            $where[] = ['create_time','<',strtotime($data['end_time'])+24*60*60];
        }

        if (isset($data['zhuangtai']) && $data['zhuangtai']!='' &&  $data['zhuangtai']!=-1 ) { //状态判断
            $zhuangtai = $data['zhuangtai'];
            $where[] = ['status','=',$data['zhuangtai']];
        }
//        dump($data);
//        dump($where);
        ...
        id,username,create_time, active_time,off_money,active_ip,point,money,grade,no_money,status,type,pid,proxy_id,group,CASE WHEN unix_timestamp(now()) - active_time < 1200 THEN active_time ELSE create_time END sort_time
